...
func (s *sqliteInternalDB) BulkReplaceMapClientData(data []MapClientData) error {
	...
	if err != nil {
		s.logger.Error(
			"failed to delete existing map client data",
			logger.Field{Key: "error", Value: err},
		)
		return fmt.Errorf("failed to delete existing map client data: %w", err)
	}
	...
	if err != nil {
		s.logger.Error(
			"failed to bulk insert map client data",
...
func (s *sqliteInternalDB) GetAllMapClientData(search string) ([]MapClientData, error) {
	var data []MapClientData

	query := s.goqu.From("map_client_data").
		Prepared(true)
	...
	if err != nil {
		s.logger.Error(
			"failed to get map client data",
			logger.Field{Key: "search", Value: search},
			logger.Field{Key: "error", Value: err},
		)
		return nil, fmt.Errorf("failed to get map client data: %w", err)
	}
