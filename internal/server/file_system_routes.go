...
func (s *Server) handleUpdateNPCFile(w http.ResponseWriter, r *http.Request) {
	...
	previousData, err := os.ReadFile(cleanPath)
	if err != nil {
		_ = utils.WriteJSONResponseWithStatus(w, http.StatusInternalServerError, map[string]interface{}{
			"errorCode": constants.ErrorCodeFileReadError,
			"context":   "file-system",
	...
	revisionID, err := s.internalDB.CreateFileRevision(tx, fileID, cleanPath, "", previousHash, currentHash, userID)
	if err != nil {
		_ = utils.WriteJSONResponseWithStatus(w, http.StatusInternalServerError, map[string]interface{}{
			"errorCode": constants.ErrorCodeInternalServerError,
			"context":   "file-system",
	...
	revisionDir := filepath.Join(s.cfg.RevisionsDirectory, fileID, strconv.FormatInt(revisionID, 10))
	...
	if err = s.internalDB.UpdateFileRevisionPath(tx, revisionID, revisionPath, userID); err != nil {
	...
	if err = s.internalDB.UpdateFileRevisionStatus(tx, revisionID, "completed", userID); err != nil {
		if removeErr := os.Remove(revisionPath); removeErr != nil {
			s.log.Error("Failed to remove revision file during cleanup", logger.Field{Key: "error", Value: removeErr})
		}
	...
	if err = s.fileEditor.WriteNPCFileData(cleanPath, npcData); err != nil {
		_ = utils.WriteJSONResponseWithStatus(w, http.StatusInternalServerError, map[string]interface{}{
			"errorCode": constants.ErrorCodeInternalServerError,
			"context":   "file-system",
...
func (s *Server) handleUpdateTextFile(w http.ResponseWriter, r *http.Request) {
	...
	if err = s.fileEditor.WriteTextFileData(cleanPath, req.Content); err != nil {
		_ = utils.WriteJSONResponseWithStatus(w, http.StatusInternalServerError, map[string]interface{}{
			"errorCode": constants.ErrorCodeInternalServerError,
			"context":   "file-system",
			"errors":    []string{"Failed to write file: " + err.Error()},
